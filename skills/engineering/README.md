...
- **[implement](./implement/SKILL.md)**: Build the work described by a spec or set of tickets, driving `/tdd` at pre-agreed seams and closing out with `/code-review` and staging only the changes made for that work.
- **[commit-message](./commit-message/SKILL.md)**: Draft Conventional Commit messages from staged changes, recommending separate commits for clearly independent changes.
